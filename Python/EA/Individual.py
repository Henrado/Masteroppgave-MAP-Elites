...
class Individual_twoLock:
    ...
    def _initControllers(self, controller, genom_raw):
        ...
        if issubclass(controller, SineController):
        ...
        elif issubclass(controller, SineControllerUfq):
            A = np.array(genom_raw[0:3])
            ...
            A_all = np.concatenate([A for _ in range(self.COUNT_LEG)])[:, np.newaxis]
            genom = np.concatenate([A_all, rest], axis=1).reshape((self.COUNT_LEG, self.ACTUATOR_LEG, self.PARAMS_ACTUATORS))
        elif issubclass(controller, TanhControllerWOffFq):
            A = np.array(genom_raw[0:3])
            F = np.array(genom_raw[3:6])
            rest = np.array(genom_raw[6:]).reshape((self.COUNT_LEG*self.ACTUATOR_LEG, 2))
...
class Individual_globalLock:
    ...
    def _initControllers(self, controller, genom_raw):
        ...
        #  ...]
        if issubclass(controller, SineController):
            A = np.array([genom_raw[0] for _ in range(self.ACTUATOR_LEG)])
            F = np.array([genom_raw[1] for _ in range(self.ACTUATOR_LEG)])
            rest = np.array(genom_raw[2:]).reshape((self.COUNT_LEG*self.ACTUATOR_LEG, controller.get_count_possibleLoc()))

            A_all = np.concatenate([A for _ in range(self.COUNT_LEG)])[:, np.newaxis]
            F_all = np.concatenate([F for _ in range(self.COUNT_LEG)])[:, np.newaxis]
            genom = np.concatenate([A_all, F_all, rest], axis=1).reshape((self.COUNT_LEG, self.ACTUATOR_LEG, 4))
        elif issubclass(controller, SineControllerUfq):
            A = np.array([genom_raw[0] for _ in range(self.ACTUATOR_LEG)])
            rest = np.array(genom_raw[1:]).reshape((self.COUNT_LEG*self.ACTUATOR_LEG, 2))

            A_all = np.concatenate([A for _ in range(self.COUNT_LEG)])[:, np.newaxis]
            genom = np.concatenate([A_all, rest], axis=1).reshape((self.COUNT_LEG, self.ACTUATOR_LEG, self.PARAMS_ACTUATORS))
        elif issubclass(controller, TanhController):
            A = np.array([genom_raw[0] for _ in range(self.ACTUATOR_LEG)])
            rest = np.array(genom_raw[1:]).reshape((self.COUNT_LEG*self.ACTUATOR_LEG, controller.get_count_possibleLoc()))
            ...
            A = np.array([genom_raw[0] for _ in range(self.ACTUATOR_LEG)])
            F = np.array([genom_raw[1] for _ in range(self.ACTUATOR_LEG)])
            rest = np.array(genom_raw[2:]).reshape((self.COUNT_LEG*self.ACTUATOR_LEG, 2))
